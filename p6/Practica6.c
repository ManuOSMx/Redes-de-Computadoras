...
void yellow(){
	printf("\033[93m");//texto amarillo
}

void red() {
	printf("\033[31m");//texto rojo
}
void blue() {
	printf("\033[34m");//texto Azul
}

void MACColor() {
	printf("\033[0m");
	printf("\033[44m");//MAC DESTINO RESALTAR
}

void MACo() {
	printf("\033[0m");
	printf("\033[42m");//MAC ORIGEN VERDE
}

void IPo() {
	printf("\033[0m");
	printf("\033[46m");//IP ORIGEN AZUL CLARO
}

void tramaColor() {
	printf("\033[0m");
	printf("\033[45m");//TRAMA	
}

void resetColors() {
	printf("\033[0m");//blanco
	printf("\033[49m");//Quitar Subrayado
}
...
void estructuraTrama(unsigned char *trama){
    ...
    unsigned char codO[2]={0x00,0x01};//codigo de operacion
...
void recibirTrama(int ds, unsigned char *trama){
    ...
    while(1){
        ...
        if(tam==-1){
            ...
            if(!(memcmp(trama+0,MACorigen,6) || memcmp(trama+12,etherARP,2)) && !(memcmp(trama+20,opcodeRes,2)|| memcmp(trama+28,IPdestino,4))){
                tramaColor();
